...
if __name__ == '__main__':
    ...
    u_c_np = np.array(u_c)
    T_mhe = 0.2
    N_MHE = 6  # estimation horizon shorter than v1 
    ...
    mhe_opt.subject_to(mhe_opt.bounded(-3.14, mhe_theta, 3.14))
    ...
    for i in range(N_MHE+1):
        init_state_m[i] = np.array([y_measurements[i, 0]*np.cos(y_measurements[i, 1]),
                                    ...
                                    0.0])
    init_control = np.array(u_c[:N_MHE])
    X_estimate = None
    U_estimate = None
    for i in range(y_measurements.shape[0]-N_MHE):
        mheiter = i
        mhe_opt.set_value(mhe_mes_ref, y_measurements[i:i+N_MHE+1])
        mhe_opt.set_value(mhe_u_ref, np.array(u_c[i:i+N_MHE]))
        mhe_opt.set_initial(mhe_states, init_state_m)
        mhe_opt.set_initial(mhe_controls, init_control)# (N, 2)
        ## solve the MHE
        sol = mhe_opt.solve()
        ## obtain the control input
        ...
        if U_estimate is None:
            ...
            U_estimate = np.concatenate((U_estimate, u_sol[N_MHE-1:]), axis=0)
        if X_estimate is None:
            X_estimate = state_sol[N_MHE:]
        else:
            X_estimate = np.concatenate((X_estimate, state_sol[N_MHE:]), axis=0)
        # shift trajectories to initialize the next step
        init_state = np.concatenate((state_sol[1:], state_sol[-1:]))
        init_control = np.concatenate((u_sol[1:], u_sol[-1:]))
    print(mheiter)
